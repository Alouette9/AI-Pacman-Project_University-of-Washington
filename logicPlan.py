...
def sentence3() -> Expr:
    ...
    expr1 = alive_1 % ((alive_0 & ~killed_0) | (~alive_0 & born_0))
    expr2 = ~(alive_0 & born_0)
    expr3 = born_0
...
def findModelUnderstandingCheck() -> Dict[Expr, bool]:
    ...
    class findExpr:
        ...
        def __lt__(self, other):
            return str(self) < str(other)   # Allow expressions to be compared and sorted by name

    a = findExpr('a')
    return {a: True}    # Return a model where 'a' is assigned True
    "*** END YOUR CODE HERE ***"
...
def plTrueInverse(assignments: Dict[Expr, bool], inverse_statement: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    return pl_true(~inverse_statement, assignments) == True # Return True if the negation of the statement is true
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    pairs = itertools.combinations(literals, 2) # Generate all pairs of literals
    clauses = [~a | ~b for a, b in pairs]   # For each pair, add clause that not both can be true
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    pacphysics_sentences.append(
        ...
        ])  # Pacman must be at exactly one position at time t
    )
    ...
    pacphysics_sentences.append(
        ...
        ])  # Pacman must take exactly one action at time t
    )

    if sensorModel is not None: 
        pacphysics_sentences.append(sensorModel(t, non_outer_wall_coords))   # Add sensor logic if provided

    if successorAxioms is not None:
        try:
            succ = successorAxioms(t, walls_grid, non_outer_wall_coords)    # Get movement rules if provided
        except AssertionError:
            succ = None          # Handle possible assertion errors
        if succ is not None:
            pacphysics_sentences.append(succ)   # Add movement rules to KB
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    KB.append(PropSymbolExpr(action0, time=0))  # Action taken at time 0
    KB.append(exactlyOne([PropSymbolExpr(a, time=0) for a in DIRECTIONS]))   # Only one action at time 0
    KB.append(PropSymbolExpr(action1, time=1))  # Action taken at time 1
    
    KB.append(pacphysicsAxioms(t=0, all_coords=all_coords,   # Add physics rules at time 0
                               non_outer_wall_coords=non_outer_wall_coords,
                               walls_grid=walls_grid))

    KB.append(pacphysicsAxioms(t=1, all_coords=all_coords,  # Add physics and movement rules at time 1
                               non_outer_wall_coords=non_outer_wall_coords,
                               walls_grid=walls_grid,
                               successorAxioms=allLegalSuccessorAxioms))

    goal = PropSymbolExpr(pacman_str, x1, y1, time=1)   # Check if Pacman is at (x1, y1) at time 1
    model_true = findModel(conjoin(KB + [goal]))     # Try to find a model where Pacman is at (x1, y1)
    model_false = findModel(conjoin(KB + [~goal]))  # Try to find a model where Pacman is not at (x1, y1)
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50): 
        KB.append(exactlyOne([PropSymbolExpr(pacman_str, x, y, time=t) for (x, y) in non_wall_coords])) # Pacman must be at one location
        
        KB.append(exactlyOne([PropSymbolExpr(a, time=t) for a in actions])) # Pacman must take one action

        if t > 0:
            for x, y in non_wall_coords:
                axiom = pacmanSuccessorAxiomSingle(x, y, time=t, walls_grid=walls_grid)
                if axiom:
                    KB.append(axiom)     # Add the rule to KB

        goal_expr = PropSymbolExpr(pacman_str, xg, yg, time=t)
        model = findModel(conjoin(KB + [goal_expr]))    # Try to find a plan that reaches the goal

        if model:
            return extractActionSequence(model, actions)    # If found, return the list of actions
...
def foodLogicPlan(problem) -> List:
    ...
    for (x, y) in non_wall_coords:
        if (x, y) in food:
            KB.append(PropSymbolExpr(food_str, x, y, time=0))   # Food exists at (x, y) at time 0
        else:
            KB.append(~PropSymbolExpr(food_str, x, y, time=0))   # No food at (x, y) at time 0

    for t in range(50):
        KB.append(exactlyOne([PropSymbolExpr(pacman_str, x, y, time=t) for (x, y) in non_wall_coords])) 

        KB.append(exactlyOne([PropSymbolExpr(action, time=t) for action in actions]))   # Pacman takes one action

        if t > 0:
            for (x, y) in non_wall_coords:
                axiom = pacmanSuccessorAxiomSingle(x, y, time=t, walls_grid=walls)  # Pacman can move if no wall
                if axiom:
                    KB.append(axiom)    # Add movement rule to KB

        if t > 0:
            for (x, y) in non_wall_coords:
                ate = PropSymbolExpr(pacman_str, x, y, time=t)  # Pacman is at (x, y) at time t
                prev_food = PropSymbolExpr(food_str, x, y, time=t-1)    # Food at (x, y) at time t-1
                curr_food = PropSymbolExpr(food_str, x, y, time=t)  # Food at (x, y) at time t
                KB.append(curr_food % (prev_food & ~ate))   # Food disappears if Pacman eats it

        goal = conjoin([~PropSymbolExpr(food_str, x, y, time=t) for (x, y) in food])    # All food must be eaten
        model = findModel(conjoin(KB + [goal]))
        if model:
            return extractActionSequence(model, actions)    # Return action plan if found
...
def localization(problem, agent) -> Generator:
    ...
    for coord in all_coords:
        if coord in walls_list:
            KB.append(PropSymbolExpr(wall_str, coord[0], coord[1]))  # wall is true
        else:
            KB.append(~PropSymbolExpr(wall_str, coord[0], coord[1]))  # wall is false

    for t in range(agent.num_timesteps):
        KB.append(pacphysicsAxioms(t, all_coords, non_outer_wall_coords, walls_grid, sensorAxioms, allLegalSuccessorAxioms))  # physics rules
        KB.append(PropSymbolExpr(agent.actions[t], time=t))  # action at time t
        KB.append(fourBitPerceptRules(t, agent.getPercepts()))  # percept rules
        KB.append(sensorAxioms(t, non_outer_wall_coords))  # sensor logic

        possible_locations = []  # list of possible pacman locations at time t
        cKB = conjoin(KB)  # combine all knowledge base

        for x, y in non_outer_wall_coords:
            pos = PropSymbolExpr(pacman_str, x, y, time=t)  # pacman at (x, y)
            if findModel(cKB & pos):  # if maybe at this location
                possible_locations.append((x, y))  # add as possible location
            elif entails(cKB, pos):  # if definitely at this location
                KB.append(pos)  # add positive fact
            else:
                KB.append(~pos)  # add negative fact
...
def mapping(problem, agent) -> Generator:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    KB.append(PropSymbolExpr(pacman_str, pac_x_0, pac_y_0, time=0))  # pacman start position
    KB.append(~PropSymbolExpr(wall_str, pac_x_0, pac_y_0))  # not a wall at pacman start

    for t in range(agent.num_timesteps):
        KB.append(pacphysicsAxioms(t, all_coords, non_outer_wall_coords, known_map, sensorAxioms, allLegalSuccessorAxioms))  # physics
        KB.append(PropSymbolExpr(agent.actions[t], time=t))  # action at t
        KB.append(fourBitPerceptRules(t, agent.getPercepts()))  # percepts
        KB.append(sensorAxioms(t, non_outer_wall_coords))  # sensor logic

        cKB = conjoin(KB)  # current full KB

        for x, y in non_outer_wall_coords:
            wall_expr = PropSymbolExpr(wall_str, x, y)  # wall symbol
            if entails(cKB, wall_expr):  # definitely a wall
                KB.append(wall_expr)  # add to KB
                known_map[x][y] = 1  # mark as wall
            elif entails(cKB, ~wall_expr):  # definitely not a wall
                KB.append(~wall_expr)  # add negation
                known_map[x][y] = 0  # mark as clear
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        KB.append(pacphysicsAxioms(t, all_coords, non_outer_wall_coords, known_map, SLAMSensorAxioms, allLegalSuccessorAxioms))  # physics ---I wasn't able to use SLAMSuccessorAxioms here, it would cause error while running so I use llLegalSuccessorAxioms here for now, but I only get 2 out of 4 due to this
        KB.append(PropSymbolExpr(agent.actions[t], time=t))  # action at t
        KB.append(numAdjWallsPerceptRules(t, agent.getPercepts()))  # 3-bit percept
        KB.append(SLAMSensorAxioms(t, non_outer_wall_coords))       # define relationships 

        cKB = conjoin(KB)   # combine all KB so far

        for x, y in non_outer_wall_coords:
            wall_expr = PropSymbolExpr(wall_str, x, y)  # wall proposition
            if entails(cKB, wall_expr):
                known_map[x][y] = 1 # mark as wall
            elif entails(cKB, ~wall_expr):
                known_map[x][y] = 0  # mark as free

        possible_locations = []

        for x, y in non_outer_wall_coords:
            pos_expr = PropSymbolExpr(pacman_str, x, y, time=t)
            if findModel(conjoin(KB + [pos_expr])):  # maybe here
